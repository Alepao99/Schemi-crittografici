...
public class Shamir1outof3Reconstruction {
	...
	public static BigInteger Reconstruct(int i1, BigInteger si1, int i2, BigInteger si2) {
		...
		Lambdai1 = tmp; // Lambda_{i1}=i2/(i2-i1) mod q - note: the division is modular division, that
						// is a/b is a*c mod q where c is the inverse of b modulo q
...
// symmetrically we compute Lambda_{i2}=i1/(i1-i2) mod q
		...
		reconstructedSecret = reconstructedSecret.add(si2.multiply(Lambdai2).mod(q)).mod(q); // si1*Lambda_{i1}+si2*Lambda_{i2}
																								// that (see slides) is
																								// equal to the secret
	...
	public static void main(String[] args) throws Exception {
		...
		int i1 = Integer.parseInt(args[0]); // the Reconstruction server receives as input two integers corresponding to
