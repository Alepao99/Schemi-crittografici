    ...
    public static void BuildTree(MerkleTreeNode[] leaves, int num, int numinputs) { // num power of two
        ...
        if (num == 1) {
            ...
            for (int i = 0; i < num / numinputs; i++) {
                ...
                if (numinputs == 2) {
                    if (leaves[j + 0].value.compareTo(q) >= 1) {
                        l3 = BigInteger.valueOf(1);
                    } else {
                        l3 = BigInteger.valueOf(0);
                    }
                    if (leaves[j + 1].value.compareTo(q) >= 1) {
                        l4 = BigInteger.valueOf(1);
                    } else {
                        l4 = BigInteger.valueOf(0);
                    }
                } else {
                    l3 = leaves[j + 2].value;
                    l4 = leaves[j + 3].value;
